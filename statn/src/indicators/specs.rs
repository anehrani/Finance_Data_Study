use anyhow::Result;
use crate::trend::ma::compute_indicators as compute_ma_indicator;
use crate::oscillators::rsi::rsi;
use crate::oscillators::macd::{macd_histogram, MacdConfig, ema};
...
fn compute_targets(prices: &[f64], start_idx: usize, n_cases: usize) -> Vec<f64> {
    (0..n_cases)
        .map(|i| {
            let idx = start_idx + i;
